...
bool areVectorFuzzyEqual(Vector2f a, Vector2f b)
{
    areFuzzyEqual(a.x, b.x);
    areFuzzyEqual(a.y, b.y);
}

Vector2f getImpuls(vector<Ball> &balls)
{
    const float mass = 10;
    Vector2f impuls;
    for (int i = 1; i < size(balls); ++i)
    {
        impuls += mass * balls[i].speed;
    }
    return impuls;
}

float getEnergy(vector<Ball> &balls)
{
    const unsigned mass = 10;
    float energy;
    for (int i = 1; i < size(balls); ++i)
    {
        energy += mass * (pow(balls[i].speed.x, 2) + pow(balls[i].speed.y, 2));
    }
    return energy;
}
...
void update(vector<Ball> &balls, const float deltaTime, const unsigned WINDOW_WIDTH = 800, const unsigned WINDOW_HEIGHT = 600, const unsigned BALL_SIZE = 30)
{
    ...
    Vector2f oldImpuls = getImpuls(balls);
    ...
    Vector2f newImpuls = getImpuls(balls);

    assert(areFuzzyEqual(newEnergy, oldEnergy));
    assert(areVectorFuzzyEqual(newImpuls, oldImpuls));
}
